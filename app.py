...
def _set_pair_query(value: str | None) -> None:
    if value is None:
        try:
...
def _render_detail_view() -> bool:
    ...
    if "results" not in st.session_state or "pdf_bytes" not in st.session_state:
        st.warning("先に比較を実行してください。")
        if st.button("比較画面へ戻る"):
            _set_pair_query(None)
            st.rerun()
        return True
    ...
    if pair_idx < 0 or pair_idx >= len(results):
    ...
    cache_key = f"annotated_{pair_idx}"
    if cache_key not in st.session_state:
        ...
        st.session_state[cache_key] = (ann_a, ann_b)

    ann_a, ann_b = st.session_state[cache_key]
    ...
    with col_l:
        _render_pdf_panel(
            ann_a,
            f"左: {docs[pair.idx_a].name}",
        )
    with col_r:
        _render_pdf_panel(
            ann_b,
            f"右: {docs[pair.idx_b].name}",
        )
...
if run:
    ...
    with st.spinner("PDFからテキストを抽出しています..."):
        for f in uploaded_files:
            try:
                file_bytes = f.read()
                text, ocr_used = extract_text_from_pdf(
                    file_bytes,
    ...
    st.subheader("比較結果")
    st.write(f"比較対象: {len(documents)}件 / ペア数: {len(documents) * (len(documents) - 1) // 2}")
